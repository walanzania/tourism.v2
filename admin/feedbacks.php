	...
	<div class="page-content">
		<div class="card">
		  <div class="card-body p-4">
			  <h5 class="card-title"> Narrator</h5>
			  <hr/>
			    ...
			    <div class="row">
                   ...
                   <div class="table-responsive">
					<table class="table mb-0">
						...
						<tbody>
							<?php
								$category = $connect2db->prepare("SELECT * FROM feedbacks ORDER BY id DESC");
								$category->execute();
...
<?php  
	if (isset($_GET['delete'])) {
		$user_id = $_GET['delete'];
		$deleteUser = $connect2db->prepare("DELETE FROM feedbacks WHERE id = ?");
	...
	if (isset($_GET['disable'])) {
		...
		$getStatus = $connect2db->prepare("SELECT status FROM feedbacks WHERE id = ?");
		$getStatus->execute([$user_id]);
		$status = $getStatus->fetch()['status'];
		$newStatus = ($status==1) ? 0 : 1 ;
		$updateUser = $connect2db->prepare("UPDATE feedbacks SET status = ? WHERE id = ?");
		$updateUser->execute([$newStatus, $user_id]);
		if ($updateUser) {
			echo "<script>success_noti('UPDATED ...');window.location='feedbacks';</script>";
		} else{
			echo "<script>error_noti('Error Updating User');window.location='feedbacks'</script>";
