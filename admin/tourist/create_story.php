...
<?php 
	if (isset($_POST['submit'])) {
		...
		$cat_unit = substr($_POST['title'],0,20); //$_POST['description'];
		$title = $_POST['title'];
	...
	// Updating
		...
		$updStory = $connect2db->prepare("UPDATE tbl_tour_story SET tour_title = ?, tour_description = ?, country_id = ?, state_id = ?, cities_id = ?, updated_at = ? WHERE id = ? ");
		$updStory->execute([$title,$desc,$country,$state,$city,$updated,$story_id]);
		if ($updStory) {
